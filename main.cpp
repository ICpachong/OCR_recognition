...
int main(int argc, char **argv) {
    ...
    for (int i = 0; i < I.rows * I.cols; ++i) {
        ...
        //是否遍历到最后一个像素
        if (i == (I.rows * I.cols - 1)) {
            free(temp_pix);
        } else {
            free(temp_pix->prev_pix);
        };
...
int check_hole(struct picture_pix *first_pix, Mat *I) {
    ...
    for (int m1 = 0; m1 < perhaps_hole_number+1; ++m1) {
        struct perhaps_hole *tempPerhapsHole = perhapsHole;
        perhapsHole = perhapsHole->next_perhasps_hole;
        free(tempPerhapsHole);
    }
...
void check_top_big_bottom(bool *top_big_bottom, int I_rows, int I_cols, struct picture_pix *first_pix
    ...
    *tourch_min_center_calculate_num_count = 0;
    ...
    int *left_distance_array = (int *) malloc(sizeof(int) * I_rows);
    ...
    //temp_pix 重新指向头指针。
    temp_pix = first_pix;
    int *right_distance_array = (int *) malloc(sizeof(int) * I_rows);
    //从最后一个元素往回计算
...
void *thread_check_top_big_bottom(void *data) {
    ...
    if (min_center_calculate_num <= (next_end_index_1 - next_start_index_1 - 1)) {
        ...
        if (4 == *(index_data->tourch_min_center_calculate_num_count)) {
